...
const formatMessageHtml = (message: string): string =>
  escapeHtml(message)
    .replace(/\n\n+/g, "</p><p style=\"margin:18px 0 0 0;font-size:17px;line-height:28px;\">")
    .replace(/\n/g, "<br/>");
...
export const platformReminderEmailTemplate = (
  ...
  if (params.template === "custom") {
    ...
    const messageHtml = `<p style="margin:0;font-size:17px;line-height:28px;">${formatMessageHtml(params.customMessage || "")}</p>`;
